...
const ContactPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const refId = `IAF-MSG-${Math.floor(10000 + Math.random() * 90000)}`;
      ...
      <main className="flex-grow">
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input className="w-full px-4 py-3 rounded-lg bg-gray-50 border-gray-200 focus:border-primary focus:ring-primary focus:bg-white transition-colors placeholder-gray-400 text-gray-900" id="name" name="name" placeholder="John Doe" required type="text" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2" htmlFor="email">Email Address</label>
                      <input className="w-full px-4 py-3 rounded-lg bg-gray-50 border-gray-200 focus:border-primary focus:ring-primary focus:bg-white transition-colors placeholder-gray-400 text-gray-900" id="email" name="email" placeholder="john@example.com" required type="email" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2" htmlFor="subject">Subject</label>
                    <input className="w-full px-4 py-3 rounded-lg bg-gray-50 border-gray-200 focus:border-primary focus:ring-primary focus:bg-white transition-colors placeholder-gray-400 text-gray-900" id="subject" name="subject" placeholder="How can we help?" required type="text" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2" htmlFor="message">Message</label>
                    <textarea className="w-full px-4 py-3 rounded-lg bg-gray-50 border-gray-200 focus:border-primary focus:ring-primary focus:bg-white transition-colors placeholder-gray-400 text-gray-900 resize-none" id="message" name="message" placeholder="Write your message here..." required rows={5}></textarea>
                  </div>
                  <button className="w-full py-4 rounded-xl bg-primary hover:bg-primary-dark text-background-dark font-bold text-lg transition-all hover:shadow-lg active:scale-95 flex items-center justify-center gap-2 group" type="submit">
                    Send Message
        ...
        <section className="w-full h-[450px] bg-gray-200 relative map-container border-t border-gray-200">
          ...
          <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-white px-6 py-3 rounded-full shadow-lg border border-gray-100 flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-gray-800">Located in Wahedpur, Debidwar</span>
          </div>
        </section>
      </main>
    </div>
